...
class JoinChatViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
        ...
        ref.child("Member").queryOrderedByKey().observeSingleEvent(of: .value, with: { (snap) in
            if(snap.exists()){
                ...
                for (_,memberPost) in postsSnap{
                    //roomID取得
                    if let userID = memberPost["userID"] as? String, let pathToImage = memberPost["pathToImage"] as? String, let roomId = memberPost["roomId"] as? String{
                        //Databaseのものと比較してオーナーユーザ情報を取得
                        //owner_posstの中に入れていく
    ...
    func joinRoom(){
        ...
        let feed = ["roomID":self.roomID,"pathToImage":self.userImage,"userID":self.uid!] as [String:Any]
